...
win_length_vector = c(10, 20, 30, 40, 50)
...
fun_1_cp_detection %>%
  filter(variables %in% c("1. Input dataset", "3. 2nd derivative")) %>%
...
=======
  ...
  fun_1_cp_detection = map_dfr(.x = win_length_vector,
                               .f = ~multi_var_ts_gradient_cp_detection(df = fun_1_df,
                                                             .x, "test", cp_factor = 1e2,
                                                             epsilon = 1e-9, date = FALSE))
  fun_1_cp_detection_example = map_dfr(.x = 60,
                               .f = ~window_length_constrain(df = fun_1_df,
                                                             .x, cp_factor = 1e2,
  ...
  fun_1_cp_detection %>%
    filter(variables %in% c("Test dataset", "2nd derivative", "cp_ratio")) %>%
    ggplot(aes(x = index, y = value)) +
    geom_line(aes(colour=variables), lwd = 1.5)+
    geom_vline(data = filter(fun_1_cp_detection,
                             cp==TRUE, variables == "Test dataset"),
